...
- Регистрирует устройства по имени
- Обеспечивает ручное управление устройствами по имени
- Ведет журнал событий с обработкой ошибок
- Проверяет наличие подписчиков перед вызовом событий
...
- Имеют уникальное имя (`Name`) для идентификации
- Подписываются на события контроллера через специфичные методы-обработчики
...
2. Проверяется наличие подписчиков на событие
3. Событие через делегат передается всем подписанным устройствам
4. Каждое устройство обрабатывает событие независимо через свой метод-обработчик
5. Все действия и ошибки логируются с указанием обработчика
6. Ошибки в одном обработчике не прерывают выполнение других
...
- ✅ **Логирование событий**
- ✅ **LINQ для работы с коллекциями** (`FirstOrDefault`)
- ✅ **Свойства интерфейсов** (автосвойства)
